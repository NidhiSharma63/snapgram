...
import type { IPost } from "@/constant/interfaces";
...
const PostDetails = () => {
  ...
  const { useGetPostById, useDeletePost, useGetAllPostStatic } = usePost();
  ...
  const { data: user, isPending: isUserLoading } = useGetUserById(
    post?.userId || ""
  );
  ...
  const {
    mutateAsync: removePostFromSave,
    isPending: isRemovingPostFromSaveCollection,
  } = useRemoveSave();
  ...
  const {
    mutateAsync: removePostFromLike,
    isPending: isRemovingPostFromLikeCollection,
  } = useRemoveLike();
  const { data: likePosts } = useGetAllLike();
  // get all post
  const { data: allPosts, isPending: isLoadingAllPost } = useGetAllPostStatic();
    ...
    if (post && userDetails) {
      ...
      try {
        if (savePosts?.[0]?.postId.includes(post._id)) {
          await removePostFromSave({
            postId: post._id,
            userId: userDetails?._id,
          });
        }
        if (likePosts?.[0]?.postId.includes(post._id)) {
          await removePostFromLike({
            postId: post._id,
            userId: userDetails?._id,
          });
        }
