...
export default function Profile() {
	...
	const { data: allPostOfUser, isPending: isUserPostLoading } =
		useGetUserAllPost(id?.replace(":", "") || "");
	const { useGetAllLike } = useLikePost();
	const { data: allLikePostIds } = useGetAllLike();
	const { data: likePosts, isFetching: isFecthingLikePost } = useGetPostByIds(
		allLikePostIds?.[0]?.postId,
	);
				...
				<div className="flex xl:flex-row flex-col max-xl:items-center flex-1 gap-7">
					...
					<div className="flex flex-col flex-1 justify-between md:mt-2">
						<div className="flex flex-col w-full">
							...
							<p className="small-regular md:body-medium dark:text-light-3 text-center xl:text-left">
								{data.email}
							</p>
							<p className="small-medium  text-center xl:text-left mt-2 max-w-screen-sm">
								Bio
							</p>
							<p className="small-medium md:base-medium text-center xl:text-left mt-1 max-w-screen-sm">
								{data.bio}
							</p>
						</div>
						<StatBlock data={data} />
						...
						<div className={`${userDetails?._id !== data._id && "hidden"}`}>
							...
							>
								<img
									...
									height={20}
								/>
								<p className="flex whitespace-nowrap small-medium dark:text-white text-black">
									Edit Profile
								</p>
							</Link>
						</div>
